...
function updateRoomPreview() {
  ...
  const preview = $('roomGeneratorCleanPreview');
  if (!preview) return;
...
function setRoomFormBusy(isBusy) {
  const loading = $('roomGeneratorInlineLoading');
...
function generateRoomsFromForm() {
  ...
  for (let index = 0; index < capacity; index += 1) {
    createOrUpdateRoom({
      roomNo: baseRoom,
      bedCode: capacity === 1 ? '' : bedCodeFromIndex(index),
      capacity: 1,
      type,
      status,
      floor: '',
...
function getRoomFilter() {
  return $('roomStatusFilter')?.value || 'all';
}
...
function selectFilteredRooms() {
  document.querySelectorAll('.room-check').forEach((checkbox) => {
...
function bulkSetRoomStatus(status) {
  ...
  ids.forEach((id) => {
    ...
    if (!room) return;
    if (status === 'bersih' && roomOccupant(id)) {
      skipped += 1;
      return;
    }
    room.status = status;
...
function importRooms(rows) {
  ...
  rows.forEach((row) => {
    ...
    createOrUpdateRoom({
      roomNo,
      bedCode: text(row.bed_code || row.kode_bed || row.bed).toUpperCase(),
      capacity: Number(row.capacity || row.kapasitas || 1),
      type: text(row.type || row.tipe || 'Single'),
      status: normalizeStatus(row.status || 'bersih'),
      floor: text(row.floor || row.lantai),
      building: text(row.building || row.gedung),
      note: text(row.note || row.catatan),
    });
    count += 1;
  });
  saveData(STORAGE_KEYS.rooms, state.rooms);
...
function renderDashboard() {
  const counts = ROOM_STATUSES.reduce((acc, status) => {
  ...
  if ($('roomStatusCards')) {
    $('roomStatusCards').innerHTML = ROOM_STATUSES.map((status) => `<div class="mini-status ${statusClass(status)}"><strong>${counts[status] || 0}</strong><span>${status}</span></div>`).join('');
  }
  ...
  if ($('mealTodayTable')) {
    $('mealTodayTable').innerHTML = todayMeals.length
      ? todayMeals.slice(-10).reverse().map((meal) => `<tr><td>${meal.guestName}</td><td>${meal.type}</td><td>${meal.time}</td></tr>`).join('')
...
function renderRooms() {
  ...
  if ($('roomOverviewGrid')) {
    $('roomOverviewGrid').innerHTML = state.rooms.map((room) => {
      const occupant = roomOccupant(room.id);
      return `<div class="room-card ${room.status}"><div><strong>${roomLabel(room)}</strong><p>${room.type}</p></div>${statusBadge(room.status)}<small>${occupant ? occupant.name : 'Kosong'}</small></div>`;
    }).join('');
  }
  ...
  if ($('brokenRoomCards')) {
    $('brokenRoomCards').innerHTML = brokenRooms.length
      ? brokenRooms.map((room) => `<div class="guest-card"><div class="guest-card-head"><div><h3>${roomLabel(room)}</h3><p>${room.type}</p></div>${statusBadge(room.status)}</div><p class="note-text">Tidak tampil di pilihan check in.</p><div class="card-actions"><button class="secondary-btn no-margin" onclick="setRoomStatus('${room.id}','kotor')">Set Kotor</button><button class="primary-btn" onclick="setRoomStatus('${room.id}','bersih')">Set Bersih</button></div></div>`).join('')
...
function initRoomsMenu() {
  ...
  $('roomNo')?.addEventListener('input', updateRoomPreview);
  $('roomCapacity')?.addEventListener('input', updateRoomPreview);
  $('roomCapacity')?.addEventListener('change', updateRoomPreview);
  ...
  $('roomForm')?.addEventListener('submit', (event) => {
    ...
    requestAnimationFrame(() => {
      const total = generateRoomsFromForm();
      if (total > 0) {
        $('roomForm').reset();
  ...
  $('roomFile')?.addEventListener('change', handleRoomFileUpload);
  ...
  document.querySelectorAll('.tab-btn').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      $(`roomTab${currentRoomTab[0].toUpperCase()}${currentRoomTab.slice(1)}`)?.classList.add('active');
    });
  });
